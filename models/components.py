...
class MixtureOfExperts(nn.Module):
    ...
    def __init__(
        ...
        top_k: int = 2,
        dropout: float = 0.1,
        load_balancing_weight: float = 0.01  # different expert
    ):
        super().__init__()
        self.num_experts = num_experts
        self.top_k = top_k
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]: # process input through MoE layer 
...
# auxilary loss helps ensure that all experts are utilized more evenly 